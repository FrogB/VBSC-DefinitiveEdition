...
//	public static var animatedShaders:Map<String, DynamicShaderHandler> = new Map<String, DynamicShaderHandler>();
	...
	//	timeBar.createFilledBar(0xFF000000, 0xFFFFFFFF);
		...
		healthBarBG = new AttachedSprite('healthBarNew'); // ignore the healthBar image in the images folder, thats just a backup one.
		...
		healthBarOverlay = new FlxSprite().loadGraphic(Paths.image('healthBarOverlay')); // in the files this image wasnt there so i had to put it in hopefully it doesnt look weird once the game compiles-
		...
		if(ClientPrefs.downScroll)
			healthBarOverlay.y = 50;

		// i would've added a animated healthbar but im not sure how you put it in source code considering it has multiple frames in its png file
		...
		scoreTxt.setFormat(Paths.font("comicsanslol.ttf"), 17, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(SONG.credit != null) {
			watermarkTxt = new FlxText(10, FlxG.height - 28, 0, 'By ' + SONG.credit + ' - ' + SONG.song + " - " + CoolUtil.difficultyString() + " - Strident Crisis - FrogB's Cut: BETA 3", 74);
			...
			watermarkTxt = new FlxText(10, FlxG.height - 28, 0, SONG.song + " - " + CoolUtil.difficultyString() + " - Strident Crisis - FrogB's Cut: BETA 3", 74);
			watermarkTxt.scrollFactor.set();
			watermarkTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			watermarkTxt.size = 18;
			watermarkTxt.updateHitbox();
		...
		/*if(songName == 'ornery')
		...
		else if(songName == 'unfairness') // i dont think these are necessary anymore since i think the people used the screw you text feature in the chart editor.
			{
				watermarkTxt.y = FlxG.height - 50;
				screwYouTxt = new FlxText(10, FlxG.height - 28, 0, "Ghost Tapping is forced off! Fuck you!", 74);
		...
		else if(songName == 'opposition')
			{
				...
				watermarkTxt.y = FlxG.height - 50;
				screwYouTxt = new FlxText(10, FlxG.height - 28, 0, "Fuck you!", 74);
				screwYouTxt.scrollFactor.set();
				screwYouTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				screwYouTxt.size = 18;
				screwYouTxt.updateHitbox();
		...
		comboCounterTxt = new FlxText(10000000, FlxG.height - 244, 0, "Combo: " + combo, 70); //NAH THEY DIDNT ADD THIS? WHAT?
		...
		healthBarOverlay.cameras = [camHUD]; // strange idk why in the cinematics event it takes away the overlay
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(tmr.loopsLeft % 4 == 0 || tmr.loopsLeft % 4 == 2) {
...
Lib.application.window.resize(width, height);*/
		...
		if(ratingString == '?') {
			scoreTxt.text =	'NPS: ' + nps + ' (Max 0) | Score: ' + songScore + ' | Phone Breaks: ' + songMisses + ' | Accuracy: 0% | N/A';
		}
		else
		{
			scoreTxt.text =  'NPS: ' + nps + ' (Max ' + maxnps + ') | Score: ' + songScore + ' | Phone Breaks: ' + songMisses + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%' + ' | (' + ratingFC + ') ' + ratingString;
		}
		if(practiceMode)
			{
				scoreTxt.text = 'NPS: ' + nps + ' (Max ' + maxnps + ') | Score: ' + songScore + ' | Phone Breaks: ' + songMisses + ' | Practice Mode ';
			}
		...
		if (iconP1.animation.frames == 3) { // stolen from os engine lol (sorry weuz)
				...
				if(!usedBotplayBefore)
					{
						...
						if(curSong == 'Opposition-alpha2' && storyDifficulty >= 1)
							{
								FlxG.save.data.oppoalpha2Unlocked = true;
								trace("unlocked opposition alpha 2 by maeving FUNI AHAHA JOE MAMA DEEZ NUTS IN YO MOUTH");
							}
						if(storyWeek == 3)
							{
								{
