...
class ExtrasMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		versionShit.setFormat("Comic Sans MS Bold", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
		var watermarkMenuTxt:FlxText = new FlxText(12, FlxG.height - 64, 0, "VB: SC - FrogB's Cut BETA 3 - Beta Full Release (Strident Crisis v1.5)", 12);
		watermarkMenuTxt.scrollFactor.set();
		watermarkMenuTxt.setFormat("Comic Sans MS Bold", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			if (controls.ACCEPT)
			{
				...
				{
					CoolUtil.browserLoad('https://discord.gg/TPxqSC9KwF');
				}
				else
				{
					selectedSomethin = true;
