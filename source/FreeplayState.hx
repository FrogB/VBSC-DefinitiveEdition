...
//'Blocked', 'Sweetcorn', 'Revise', 'Cob', 'Idiot', 'Apathetic', 'Screwed', 'Overthink', 'Nomophobia', 'Dismember', 'Kalampokiphobia', 'Reality-Breaking', 'Opposition', 'Cheating', 'Shit', 'Unfairness', 'Phonophobia', 'Hellbreaker'
	...
	public function goToFreeplay()
	{
		...
		for (i in 0...songs.length)
		{
			...
			else if(songs[i].songName == 'Opposition-alpha2' && FlxG.save.data.oppoalpha2unlocked != true) //old one was tutorialCompletePlaceholder2
				{
					var songText:Alphabet = new Alphabet(0, (70 * i) + 30, 'Hidden Song', true, false);
					songText.isMenuItem = true;
					songText.itemType = "D-Shape";
		...
		skipText = new FlxText(FlxG.width * 0.7, 640, 0, "", 32);
		skipText.setFormat(Paths.font("comicsanslol.ttf"), 32, FlxColor.RED, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(skipText);
	...
	override function update(elapsed:Float)
	{
		...
		if(inMainFreeplay)
			{
				...
				skipText.text = 'Hold SHIFT to skip the Character Selector';
		...
		else*/ #end if (accepted && !playingRN)
		{
			...
			if(songLowercase == 'pessimistic' && FlxG.save.data.invisibleWeekUnlocked1 != true)
				{
					...
					if(!unlockTxtThing)
						...
						youNeedToBeatTxt = new FlxText(0, 0, FlxG.width, "You need to beat Chapter 4\nto unlock this song.", 32);
						 youNeedToBeatTxt.setFormat("Comic Sans MS Bold", 32, FlxColor.WHITE, CENTER, OUTLINE, 0xFF000000);
			...
			else if(songLowercase == 'opposition-alpha2' && FlxG.save.data.oppoalpha2Unlocked != true)
			{
				FlxG.sound.play(Paths.sound('cancelMenu'), 0.7);
				if(!unlockTxtThing)
					{
					unlockTxtThing = true;
					youNeedToBeatTxt = new FlxText(0, 0, FlxG.width, "You need to press 7 on Opposition and\nbeat the secret song\nto unlock this song.", 32);
     				youNeedToBeatTxt.setFormat("Comic Sans MS Bold", 32, FlxColor.WHITE, CENTER, OUTLINE, 0xFF000000);
      				youNeedToBeatTxt.screenCenter(Y);
     				add(youNeedToBeatTxt);
					new FlxTimer().start(1, function(tmr:FlxTimer)
			...
			else if (accepted && !gotIn)
			{
				...
				FlxTween.tween(skipText, {alpha: 0}, 0.25, {ease: FlxEase.cubeIn, type:ONESHOT});
				...
				FlxTween.tween(FlxG.camera, {zoom:1.35}, 1.25, {ease: FlxEase.expoIn});
				new FlxTimer().start(1.25, function(tmr:FlxTimer)
					{
						...
						if (FlxG.keys.pressed.SHIFT) LoadingState.loadAndSwitchState(new PlayState()); // thye can hold shift to skip the selection shit.
		...
		else if(controls.RESET)
		{
			...
			else if(songs[curSelected].songName == 'Opposition-alpha2' && FlxG.save.data.oppoalpha2Unlocked != true)
				{
